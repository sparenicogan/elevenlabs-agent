...
- [x] T005b Bootstrap the state bucket outside Terraform, since Terraform cannot create the bucket it is already using as a backend: `aws s3api create-bucket --bucket elevenlabs-agent-tfstate-199013204701 --region eu-central-1 --create-bucket-configuration LocationConstraint=eu-central-1`, then enable versioning on it. Record the command in `README.md` so the step is reproducible
